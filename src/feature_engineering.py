...
def engineer_date_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # days since ref date — captures long-run box office inflation trend
...
def engineer_cast_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # popularity > 20 is roughly a-list on tmdb
...
def engineer_budget_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # log transform reduces skew — budget spans several orders of magnitude
...
def engineer_historical_features(df: pd.DataFrame) -> pd.DataFrame:
    # historical revenue for directors and lead actors — no data leakage
    # only uses films released BEFORE the current one
    df = df.copy()
...
def engineer_title_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # catches "2", "Part II", "Chapter 3", etc.
...
def engineer_marketing_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "days_trailer_before_release" in df.columns:
        df["early_marketing"] = (df["days_trailer_before_release"] > 180).astype(int)  # 6+ months out
        df["late_marketing"] = (df["days_trailer_before_release"] < 30).astype(int)
...
def engineer_target_variable(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # predict log revenue — much better behaved than raw dollar amounts
...
def select_model_features(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    # only pre-release features — nothing that requires the movie to have already come out
    # excluded: vote_average, vote_count, popularity (all post-release)
    # excluded: num_translations (accumulates post-release as movie gets popular)
    numeric_features = [
        "budget", "log_budget",
